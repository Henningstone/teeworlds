...
void CChat::ConChat(IConsole::IResult *pResult, void *pUserData)
{
	...
	bool Answer = pResult->GetInteger(1) == 1;
...
void CChat::EnableMode(int Team, bool Answer)
{
	...
	if(m_Mode == MODE_NONE)
	{
		if(Team)
			...
			m_Mode = MODE_ALL;
		if(Answer && m_ChatPartner[0])
		{
			char aBuf[16+2];
			str_format(aBuf, sizeof(aBuf), "%s: ", m_ChatPartner);
			m_Input.Set(aBuf);
		}
		else
			m_Input.Clear();
...
void CChat::AddLine(int ClientID, int Team, const char *pLine)
{
	...
	while(*p)
	{
		...
		else
		{
			...
			if(Highlighted)
				str_copy(m_ChatPartner, m_aLines[m_CurrentLine].m_aName, sizeof(m_ChatPartner));
		}
